...
class Pieces:
    def __init__(self, x, y, colourxy, colourx, coloury, health=1): #__init__(self,)
        self.x = x
        self.y = y
        self.colourxy = round((colourx+coloury)/2)
    ...
    def draw(self, WIN):
        pygame.draw.rect(WIN, (self.colourxy,self.colourx,self.coloury), (self.x, self.y, 50, 50)) #or..50), #) for border
...
def main():
    ...
    while run:
        ...
        vinayx = Piece1.x
        Piece1.colourx = round((vinayx/1280)*255)
        vinayy = Piece1.y
        Piece1.coloury = round((vinayy/720)*255)
   
                
        keys = pygame.key.get_pressed()
        if keys[pygame.K_a] and Piece1.x - player_vel*2 > 0: 
            Piece1.x -= player_vel #LEFT (#4arrows)

        if keys[pygame.K_d]and Piece1.x + player_vel*2 + 50 < WIDTH: #RIGHT #Piece1.get_width(
            Piece1.x += player_vel #right is posotive

        if keys[pygame.K_s] and Piece1.y + player_vel*2 + 50 < HEIGHT: #DOWN
            Piece1.y += player_vel #down is postive

        if keys[pygame.K_w] and Piece1.y - player_vel*2 > 0: #UP
            Piece1.y -= player_vel
